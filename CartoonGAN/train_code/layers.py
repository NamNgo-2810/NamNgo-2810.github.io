...
def spectral_norm(w, iteration=1):
    ...
    u = variable_scope.get_variable("u", [1, w_shape[-1]], initializer=tf.random_normal_initializer(), trainable=False)
    u_hat = u
    v_hat = None
...
def conv_spectral_norm(x, channel, k_size, stride=1, name='conv_snorm'):
    with variable_scope.variable_scope(name):
        w = variable_scope.get_variable("kernel", shape=[k_size[0], k_size[1], x.get_shape()[-1], channel])
        b = variable_scope.get_variable("bias", [channel], initializer=tf.constant_initializer(0.0))
...
def self_attention(inputs, name='attention', reuse=False):
    with variable_scope.variable_scope(name, reuse=reuse):
        ...
        gamma = variable_scope.get_variable("gamma", [1], initializer=tf.constant_initializer(0.0))
