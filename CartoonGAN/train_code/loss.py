...
def vggloss_4_4(image_a, image_b):
    ...
    VGG_loss = losses_impl.absolute_difference(vgg_a, vgg_b)
    h, w, c = vgg_a.get_shape().as_list()[1:]
    VGG_loss = tf.reduce_mean(VGG_loss) / (h * w * c)
    return VGG_loss


def wgan_loss(discriminator, real, fake, patch=True, channel=32, name='discrminator', lambda_=2):
    ...
    alpha = random_ops.random_uniform([tf.shape(real)[0], 1, 1, 1], minval=0., maxval=1.)
...
def gan_loss(discriminator, real, fake, scale=1, channel=32, patch=False, name='discriminator'):
    ...
    g_loss_blur = -tf.reduce_mean(tf.math.log(fake_logit))
    d_loss_blur = -tf.reduce_mean(tf.math.log(real_logit) + tf.math.log(1. - fake_logit))

    return d_loss_blur, g_loss_blur


def lsgan_loss(discriminator, real, fake, scale=1, channel=32, patch=False, name='discriminator'):
    real_logit = discriminator(real, scale, channel, name=name, patch=patch, reuse=False)
    fake_logit = discriminator(fake, scale, channel, name=name, patch=patch, reuse=True)

    g_loss = tf.reduce_mean((real_logit - 1) ** 2)
