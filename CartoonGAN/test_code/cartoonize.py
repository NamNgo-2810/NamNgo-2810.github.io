...
def cartoonize(load_folder, save_folder, model_path):
    ...
    final_out = gf.guided_filter(input_photo, network_out, r=1, eps=5e-3)

    all_vars = variables.trainable_variables()
    gene_vars = [var for var in all_vars if 'generator' in var.name]
    saver = training.Saver(var_list=gene_vars)
    ...
    saver.restore(sess, training.latest_checkpoint(model_path))
